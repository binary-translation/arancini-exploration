...
static std::optional<po::variables_map> init_options(int argc, const char *argv[]) {
	...
	desc.add_options() //
		...
		("cxx-compiler-path", po::value<std::string>()->default_value("clang"), "Path to C++ compiler to use for translated binary") //
#else
    ...
    ("disable-fence-opt", "Disable optimizations that merge consecutive fences") //
