...
arm64_translation_context::guestreg_memory_operand(int regoff, memory_operand::address_mode mode) {
    if (regoff > 255 || regoff < -256) {
        ...
        return memory_operand(base_vreg, immediate_operand(0, value_types::u12), mode);
    } else {
        return memory_operand(context_block_reg, immediate_operand(regoff, value_types::u12), mode);
    }
}
...
void arm64_translation_context::materialise_read_mem(const read_mem_node &n) {
    ...
    for (std::size_t i = 0; i < dest_vregs.size(); ++i) {
        ...
        memory_operand mem_op(addr_vreg, immediate_operand(i * width, value_types::u12));
        switch (width) {
            case 1:
            case 8:
...
void arm64_translation_context::materialise_write_mem(const write_mem_node &n) {
    ...
    for (std::size_t i = 0; i < src_vregs.size(); ++i) {
        ...
        memory_operand mem_op(address, immediate_operand(i * width, value_types::u12));
        switch (width) {
            case 1:
            case 8:
...
void arm64_translation_context::materialise_binary_arith(const binary_arith_node &n) {
	...
	case binary_arith_op::add:
        ...
        builder_.adds(dest_regset[0], lhs_regset[0], rhs_regset[0], binop_modifier(n.val().type()));
	...
	case binary_arith_op::sub:
        ...
        builder_.subs(dest_regset[0], lhs_regset[0], rhs_regset[0], binop_modifier(n.val().type()));
	...
	case binary_arith_op::bor:
        ...
        switch (n.val().type().element_width()) {
        case 1:
            builder_.lsl(lhs_regset[0], lhs_regset[0], immediate_operand(7, value_type::u8()),
                         "shift left LSB to set sign bit of byte");
            builder_.asr(lhs_regset[0], lhs_regset[0], immediate_operand(7, value_type::u8()),
                         "shift right to fill LSB with sign bit (except for least-significant bit)");
            builder_.lsl(rhs_regset[0], rhs_regset[0], immediate_operand(7, value_type::u8()),
                         "shift left LSB to set sign bit of byte");
        ...
        case 8:
            ...
            if (n.lhs().type().type_class() == ir::value_type_class::unsigned_integer)
                builder_.uxtb(lhs_regset, lhs_regset);
            if (n.rhs().type().type_class() == ir::value_type_class::signed_integer)
                builder_.sxtb(rhs_regset, rhs_regset);
            if (n.rhs().type().type_class() == ir::value_type_class::unsigned_integer)
                builder_.uxtb(rhs_regset, rhs_regset);
            builder_.orr_(dest_regset, lhs_regset, rhs_regset);
            break;
        case 16:
            if (n.lhs().type().type_class() == ir::value_type_class::signed_integer)
                builder_.sxth(lhs_regset, lhs_regset);
            if (n.lhs().type().type_class() == ir::value_type_class::unsigned_integer)
                builder_.uxth(lhs_regset, lhs_regset);
            if (n.rhs().type().type_class() == ir::value_type_class::signed_integer)
                builder_.sxth(rhs_regset, rhs_regset);
            if (n.rhs().type().type_class() == ir::value_type_class::unsigned_integer)
                builder_.uxth(rhs_regset, rhs_regset);
        case 32:
        case 64:
            builder_.orr_(dest_regset, lhs_regset, rhs_regset);
            break;
	...
	case binary_arith_op::band:
        ...
        case 16:
            if (n.lhs().type().type_class() == ir::value_type_class::signed_integer)
                builder_.sxth(lhs_regset[0], lhs_regset[0]);
            if (n.lhs().type().type_class() == ir::value_type_class::unsigned_integer)
                builder_.uxth(lhs_regset[0], lhs_regset[0]);
        case 32:
        case 64:
            builder_.ands(dest_regset, lhs_regset, rhs_regset);
            break;
	...
	case binary_arith_op::bxor:
        {
            ...
            if (!is_vector_op)
                builder_.cmp(dest_regset[i-1], immediate_operand(0, value_type::u8()));
            else
                sets_flags = false;
        }
        break;
	...
	case binary_arith_op::cmpgt:
        ...
        builder_.cmp(lhs_regset[0], rhs_regset[0], "compare LHS and RHS to generate condition for conditional set");
        builder_.cset(dest_regset[0], get_cset_type(n.op()), "set to 1 if condition is true (based flags from the previous compare)");
		break;
	default:
	...
	case cast_op::bitcast:
        ...
        if (n.val().type().element_width() > n.source_value().type().element_width()) {
            ...
            for (std::size_t i = 0; i < src_vregs.size(); ++i) {
                ...
                builder_.lsl(src_vreg, src_vreg, move_immediate(dest_pos % n.val().type().element_width(), value_types::u6, src_vreg.type()));
            ...
            for (std::size_t i = 0; i < dest_vregs.size(); ++i) {
                ...
                builder_.lsl(src_vreg, src_vreg, move_immediate(src_pos % n.source_value().type().element_width(), value_types::u6, src_vreg.type()));
