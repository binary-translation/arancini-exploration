...
template <typename T> static immediate_operand clamped_immediate(T v, value_type t) { return immediate_operand(v & ~(1 << t.element_width()), t); }
...
template <typename T, std::enable_if_t<std::is_arithmetic<T>::value, int>> vreg_operand arm64_translation_context::mov_immediate(T imm, ir::value_type type)
{
	...
	if (actual_size <= base_type().element_width()) {
		builder_.insert_comment("Move immediate directly as > 16-bits");
		builder_.movz(reg, immediate_operand(immediate & 0xFFFF, value_type::u16()), shift_operand("LSL", immediate_operand(0, value_type::u1())));
		for (size_t i = 1; i < move_count; ++i) {
			builder_.movk(
				reg, immediate_operand(immediate >> (i * 16) & 0xFFFF, value_type::u16()), shift_operand("LSL", immediate_operand(i * 16, value_type::u16())));
		}
...
memory_operand arm64_translation_context::guestreg_memory_operand(int regoff, bool pre, bool post)
{
	memory_operand mem;
...
void arm64_translation_context::end_block()
{
	...
	builder_.mov(preg_operand(preg_operand::x0), mov_immediate(ret_, value_type::u64()));
...
void arm64_translation_context::lower(ir::node *n) { nodes_.push_back(n); }
...
void arm64_translation_context::materialise(const ir::node *n)
{
	...
	case node_kinds::write_local:
		...
		break;
	default:
		throw std::runtime_error("unknown node encountered: " + std::string(n->to_string()));
	}
...
{
	return value.type().width() == 1 || value.kind() == port_kinds::zero || value.kind() == port_kinds::carry || value.kind() == port_kinds::negative
...
void arm64_translation_context::materialise_write_pc(const write_pc_node &n)
{
	...
	builder_.str(new_pc_vregs[0], guestreg_memory_operand(static_cast<int>(reg_offsets::PC)), "write program counter");
}
...
void arm64_translation_context::materialise_br(const br_node &n) { builder_.b(n.target()->name()); }
...
void arm64_translation_context::materialise_ternary_arith(const ternary_arith_node &n)
{
	...
	if (dest_vregs.size() != lhs_vregs.size() || dest_vregs.size() != rhs_vregs.size() || dest_vregs.size() != top_vregs.size()) {
...
void arm64_translation_context::materialise_ternary_atomic(const ternary_atomic_node &n)
{
	...
	if (dest_vregs.size() != 1 || src_vregs.size() != 1 || acc_vregs.size() != 1 || addr_vregs.size() != 1) {
		throw std::runtime_error("[ARM64-DBT] Ternary atomic operations not supported for vectors - dest: " + std::to_string(n.val().type().nr_elements())
			+ " x " + std::to_string(n.val().type().element_width()) + ", top: " + std::to_string(n.top().type().nr_elements()) + " x "
			+ std::to_string(n.top().type().element_width()) + ", rhs: " + std::to_string(n.rhs().type().nr_elements()) + " x "
			+ std::to_string(n.rhs().type().element_width()) + ", addr: " + std::to_string(n.rhs().type().nr_elements()) + " x "
			+ std::to_string(n.rhs().type().element_width()));
	}
	...
	case ternary_atomic_op::cmpxchg:
		if constexpr (supports_lse) {
			...
			builder_.cas(acc_vreg, src_vreg, memory_operand(mem_addr), "write source to memory if source == accumulator, accumulator = source");
...
void arm64_translation_context::materialise_cast(const cast_node &n)
{
	...
	case cast_op::sx:
		...
		if (n.val().type().element_width() <= n.source_value().type().element_width()) {
			throw std::runtime_error("[ARM64-DBT] cannot sign-extend " + std::to_string(n.val().type().element_width()) + " to smaller size "
				+ std::to_string(n.source_value().type().element_width()));
		}

		builder_.insert_comment("Sign-extend from " + std::to_string(n.source_value().type().nr_elements()) + "x"
			+ std::to_string(n.source_value().type().element_width()) + " to " + std::to_string(n.val().type().nr_elements()) + "x"
			+ std::to_string(n.val().type().element_width()));
		...
		case 1:
			...
			builder_.lsl(dest_vreg, src_vreg, immediate_operand(7, value_type::u8()), "shift left LSB to set sign bit of byte");
			builder_.sxtb(dest_vreg, dest_vreg, "sign-extend");
			builder_.asr(
				dest_vreg, dest_vreg, immediate_operand(7, value_type::u8()), "shift right to fill LSB with sign bit (except for least-significant bit)");
			break;
		case 8:
		...
		default:
			throw std::runtime_error(
				"[ARM64-DBT] cannot sign-extend from size " + std::to_string(src_vreg.width()) + " to size " + std::to_string(dest_vreg.width()));
		}
	...
	case cast_op::bitcast:
		...
		// allocator)
		builder_.insert_comment("Bitcast from " + std::to_string(n.source_value().type().nr_elements()) + "x"
			+ std::to_string(n.source_value().type().element_width()) + " to " + std::to_string(n.val().type().nr_elements()) + "x"
			+ std::to_string(n.val().type().element_width()));
	...
	case cast_op::zx:
		// Sanity check
		if (n.val().type().element_width() <= n.source_value().type().element_width()) {
			throw std::runtime_error("[ARM64-DBT] Cannot zero-extend " + std::to_string(n.val().type().element_width()) + " to smaller size "
				+ std::to_string(n.source_value().type().element_width()));
		}

		builder_.insert_comment("Zero-extend from " + std::to_string(n.val().type().nr_elements()) + "x" + std::to_string(n.val().type().element_width())
			+ " to " + std::to_string(n.source_value().type().nr_elements()) + "x" + std::to_string(n.source_value().type().element_width()));
	...
	case cast_op::trunc:
		if (dest_vreg.width() > src_vreg.width()) {
			throw std::runtime_error(
				"[ARM64-DBT] cannot truncate from " + std::to_string(dest_vreg.width()) + " to larger size " + std::to_string(src_vreg.width()));
		}
	...
	case cast_op::convert:
		...
		if (dest_vregs.size() != 1) {
			throw std::runtime_error("[ARM64-DBT] cannot convert " + std::to_string(n.val().type().element_width()) + " because it larger than 64-bit");
		}
		...
		if (n.source_value().type().is_integer() && n.val().type().is_floating_point()) {
			...
			default:
				throw std::runtime_error("Cannot convert type: " + std::to_string(int(n.convert_type())));
			}
		} else {
		...
		break;
	default:
		throw std::runtime_error("unsupported cast operation: " + to_string(n.op()));
	}
}
...
void arm64_translation_context::materialise_csel(const csel_node &n)
{
	...
	if (cond_vregs.size() != true_vregs.size() || true_vregs.size() != false_vregs.size() || false_vregs.size() != 1) {
...
void arm64_translation_context::materialise_bit_shift(const bit_shift_node &n)
{
	...
	case shift_op::asr:
		...
		break;
	default:
		throw std::runtime_error("unsupported shift operation: " + std::to_string(static_cast<int>(n.op())));
	}
}

static inline std::size_t total_width(const std::vector<vreg_operand> &vec) { return std::ceil(vec.size() * vec[0].width()); }
...
void arm64_translation_context::materialise_bit_extract(const bit_extract_node &n)
{
	...
	for (std::size_t i = extract_start; extracted < n.length(); ++i) {
		...
		builder_.bfxil(dest_vregs[dest_idx], src_vregs[i], immediate_operand(extract_idx, value_type::u8()), immediate_operand(extract_len, value_type::u8()));
		extract_idx = 0;
...
void arm64_translation_context::materialise_bit_insert(const bit_insert_node &n)
{
	...
	for (std::size_t i = insert_start; inserted < n.length(); ++i) {
		...
		builder_.bfi(dest_vregs[i], bits_vregs[bits_idx], immediate_operand(insert_idx, value_type::u8()), immediate_operand(insert_len, value_type::u8()));
		insert_idx = 0;
		inserted += insert_len;
...
void arm64_translation_context::materialise_vector_insert(const vector_insert_node &n)
{
	...
	if (index + value_vregs.size() > dest_vregs.size())
		throw std::runtime_error("[ARM64-DBT] Cannot insert at index " + std::to_string(index) + " in destination vector");
...
void arm64_translation_context::materialise_vector_extract(const vector_extract_node &n)
{
	...
	if (index + dest_vregs.size() > src_vregs.size())
		throw std::runtime_error("[ARM64-DBT] Cannot extract from index " + std::to_string(index) + " in source vector");
...
void arm64_translation_context::materialise_internal_call(const internal_call_node &n)
{
	if (n.fn().name() == "handle_syscall") {
		...
		builder_.str(pc_vreg, guestreg_memory_operand(static_cast<int>(reg_offsets::PC)), "update program counter to handle system call");
		ret_ = 1;
