...
void instruction_builder::emit(machine_code_writer &writer)
{
	...
	for (std::size_t i = 0; i < instructions_.size(); ++i) {
		...
		for (size_t i = 0; i < insn.operand_count(); ++i) {
			...
			if (op.type() == operand_type::invalid || op.type() == operand_type::vreg || (op.type() == operand_type::mem && op.memory().is_virtual())) {
				dump(assembly);
				std::cerr << assembly.str() << '\n';
				throw std::runtime_error("Virtual register after register allocation: " + insn.dump());
			}
		}
	}
...
void instruction_builder::allocate()
{
	...
	for (auto RI = instructions_.rbegin(), RE = instructions_.rend(); RI != RE; RI++) {
		...
		auto allocate = [&allocs, &avail_physregs, &avail_float_physregs, &vreg_to_preg](operand &o, size_t idx) -> void {
			unsigned int vri;
			ir::value_type type;
			if (o.is_vreg()) {
				vri = o.vreg().index();
