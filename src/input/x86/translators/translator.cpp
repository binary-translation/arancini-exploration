...
reg_offsets translator::xedreg_to_offset(xed_reg_enum_t reg) {
    ...
    default:
        throw frontend_exception("unsupported register class when computing offset from xed");
    }
}
...
action_node *translator::write_operand(int opnum, port &value) {
	...
	case XED_OPERAND_REG2: {
		...
		case XED_REG_CLASS_GPR: {
			...
			default:
				throw frontend_exception("{}: {}: unsupported general purpose register width {}", __FILE__, __LINE__, width);
			}
		}
		...
		case XED_REG_CLASS_ZMM: {
			...
			if (!xed_classify_sse(xed_inst())) {
				...
				switch(val_len) {
					...
					default: throw frontend_exception("unsupported value width when writing X/Y/ZMM registers");
				}
        ...
        return fpu_stack_set(st_idx, value);
			...
			default:
				throw frontend_exception("unsupported x87 register type");
			}
			break;
		}

		default:
			throw frontend_exception("{}: {}: unsupported register class {}", __FILE__, __LINE__, util::to_underlying(regclass));
		}
	}
	...
	default:
		throw frontend_exception("unsupported write operand type: {}", util::to_underlying(opname));
	}
}
...
value_node *translator::read_operand(int opnum)
{
	...
	case XED_OPERAND_REG3: {
		...
		case XED_REG_CLASS_GPR:
			...
			default:
				throw frontend_exception("unsupported register size");
			}
    ...
    case XED_REG_CLASS_PSEUDOX87: {
      ...
      default:
				throw frontend_exception("unsupported pseudoX87 register type");
      }
      break;
    }

		default:
			throw frontend_exception("{}: {}: unsupported register class {}",
                                     __FILE__, __LINE__, util::to_underlying(regclass));
		}
	}
	...
	case XED_OPERAND_IMM0: {
		if (xed_decoded_inst_get_immediate_is_signed(xed_inst())){
			...
			default:
				throw frontend_exception("unsupported immediate width");
			}
		} else {
			switch (xed_decoded_inst_get_immediate_width_bits(xed_inst())) {
			case 8:
  ...
  case XED_OPERAND_MEM1: {
    ...
    case 1:
		...
		default:
			throw frontend_exception("invalid memory width in read");
		}
	}
	...
	default:
		throw frontend_exception("unsupported read operand type: {}", util::to_underlying(opname));
	}
}
...
ssize_t translator::get_operand_width(int opnum)
{
  ...
  default:
		throw frontend_exception("unsupported operand width query");
	}
}
...
value_node *translator::compute_address(int mem_idx)
{
	...
	if (xed_get_register_width_bits(base_reg) != 64 && base_reg != XED_REG_INVALID) {
		throw frontend_exception("base reg invalid size");
	}
...
action_node *translator::fpu_stack_top_move(int val) {
    ...
    if (val == 1) { // pop
        ...
        throw frontend_exception("Cannot move the FPU stack by {}, the value must be 1 or -1", val);
    }
...
void translator::write_flags(value_node *op, flag_op zf, flag_op cf, flag_op of, flag_op sf, flag_op pf, flag_op af) {
	...
	case flag_op::update:
		...
		case node_kinds::constant:
			...
			break;
		default:
			throw frontend_exception("unsupported operation node type for ZF");
		}
		break;
	default:
		break;
	...
	case flag_op::update:
		...
		case node_kinds::ternary_atomic:
			...
			break;
		default:
			throw frontend_exception("unsupported operation node type for CF");
		}
		break;
	default:
		break;
	...
	case flag_op::update:
		...
		case node_kinds::ternary_atomic:
			...
			break;
		default:
			throw frontend_exception("unsupported operation node type for OF");
		}
		break;
	default:
		break;
	...
	case flag_op::update:
		...
		case node_kinds::constant:
			...
			break;
		default:
			throw frontend_exception("unsupported operation node type for SF");
		}
		break;
	default:
		break;
...
value_node *translator::compute_cond(cond_type ct) {
	...
	case cond_type::nb:
		return builder_.insert_not(read_reg(value_type::u1(), reg_offsets::CF)->val());
	case cond_type::b:
		return read_reg(value_type::u1(), reg_offsets::CF);
	case cond_type::nz:
		return builder_.insert_not(read_reg(value_type::u1(), reg_offsets::ZF)->val());
	case cond_type::no:
		return builder_.insert_not(read_reg(value_type::u1(), reg_offsets::OF)->val());
	case cond_type::np:
		return builder_.insert_not(read_reg(value_type::u1(), reg_offsets::PF)->val());
	case cond_type::ns:
		return builder_.insert_not(read_reg(value_type::u1(), reg_offsets::SF)->val());
	case cond_type::o:
		return read_reg(value_type::u1(), reg_offsets::OF);
	case cond_type::p:
		return read_reg(value_type::u1(), reg_offsets::PF);
	case cond_type::s:
		return read_reg(value_type::u1(), reg_offsets::SF);
	default:
		throw frontend_exception("unhandled condition code: {}", util::to_underlying(ct));
	}
}
...
value_node *translator::auto_cast(const value_type &target_type, value_node *v) {
	...
	if (target_type.width() < vtype.width()) {
		...
		if (vtype.type_class() == value_type_class::signed_integer)
			...
			throw frontend_exception("auto cast not supported for type class {}", vtype.type_class());
	}
...
value_type translator::type_of_operand(int opnum) {
	...
	default:
		throw frontend_exception("unsupported operand type: {}", util::to_underlying(opname));
	}
}
