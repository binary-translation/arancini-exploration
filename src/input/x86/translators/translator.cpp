		...
		case XED_REG_CLASS_GPR:
			if (value.type().width() == 64) {
				if (value.type().type_class() == value_type_class::signed_integer) {
					return write_reg(reg_to_offset(reg), pkt()->insert_bitcast(value_type::u64(), value)->val());
				} else {
				...
				if (value.type().width() == 32) {
					// EAX behaviour
					return write_reg(reg_to_offset(reg), pkt()->insert_zx(value_type::u64(), value)->val());
				} else {
					// AX/AL behaviour
					...
					return write_reg(reg_to_offset(reg), pkt()->insert_zx(value_type::u64(), repl->val())->val());
				}
			}
...
value_node *translator::compute_address(int mem_idx)
{
	...
	if (index != XED_REG_INVALID) {
		auto scaled_index = pkt()->insert_mul(read_reg(value_type::u64(), reg_to_offset(index))->val(), pkt()->insert_constant_u64(scale)->val());
	...
	if (seg == XED_REG_FS) {
		address_base = pkt()->insert_add(address_base->val(), read_reg(value_type::u64(), reg_offsets::fs)->val());
	} else if (seg == XED_REG_GS) {
		address_base = pkt()->insert_add(address_base->val(), read_reg(value_type::u64(), reg_offsets::gs)->val());
	}
...
action_node *translator::write_reg(reg_offsets reg, port &value) { return pkt()->insert_write_reg((unsigned long)reg, value); }

value_node *translator::read_reg(const value_type &vt, reg_offsets reg) { return pkt()->insert_read_reg(vt, (unsigned long)reg); }
...
value_node *translator::compute_cond(cond_type ct)
{
	switch (ct) {
	case cond_type::nbe: {
		auto ncf = pkt()->insert_not(read_reg(value_type::u1(), reg_offsets::cf)->val());
		auto nzf = pkt()->insert_not(read_reg(value_type::u1(), reg_offsets::zf)->val());
	...
	case cond_type::no: {
		auto nof = pkt()->insert_not(read_reg(value_type::u1(), reg_offsets::of)->val());
		return nof;
	}

	case cond_type::np: {
		auto npf = pkt()->insert_not(read_reg(value_type::u1(), reg_offsets::pf)->val());
		return npf;
	}

	case cond_type::ns: {
		auto nsf = pkt()->insert_not(read_reg(value_type::u1(), reg_offsets::sf)->val());
		return nsf;
	}
