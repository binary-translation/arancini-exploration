...
action_node *translator::write_operand(int opnum, port &value)
{
	...
	case XED_OPERAND_REG2: {
		...
		case XED_REG_CLASS_YMM:
			return write_reg(reg_to_offset(reg), builder_.insert_zx(value_type::u256(), value)->val());
		case XED_REG_CLASS_ZMM:
			return write_reg(reg_to_offset(reg), builder_.insert_zx(value_type::u512(), value)->val());
