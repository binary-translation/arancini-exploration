    ...
    case XED_ICLASS_SHLD: {
        ...
        if (is_immediate_operand(1)) { // shift amount is an immediate
            ...
            if (shift_val != 0) {
                ...
                if (inst == XED_ICLASS_SHRD) {
                    ...
                    shift = builder().insert_bit_insert(
                        shift->val(),
                        builder().insert_bit_extract(
                            in->val(),
                            in->val().type().width() - shift_val,
                            shift_val
                        )->val(), in->val().type().width() - shift_val, shift_val
                    );
                } else {
                    shift = builder().insert_lsl(src->val(), amt->val());
                    shift = builder().insert_bit_insert(
                ...
                value_node *last_shift_bit;
                if (inst == XED_ICLASS_SHRD) {
                    last_shift_bit = builder().insert_bit_extract(src->val(), shift_val-1, 1);
                } else {
                    last_shift_bit = builder().insert_bit_extract(src->val(), src->val().type().width() - shift_val, 1);
                }
                ...
                if (shift_val == 1) {
                    ...
                    if (inst == XED_ICLASS_SHRD) {
                        auto msb = builder().insert_bit_extract(src->val(), src->val().type().width() - 1, 1);
                        auto new_msb = builder().insert_bit_extract(shift->val(), shift->val().type().width() - 1, 1);
            ...
            auto to_clear = builder().insert_sub(
                builder().insert_constant_i(in->val().type(), in->val().type().width())->val(),
                amt->val()
            );
            if (inst == XED_ICLASS_SHRD) {
                shift = builder().insert_lsr(src->val(), amt->val());
            ...
            if (inst == XED_ICLASS_SHRD) {
                auto old_msb = builder().insert_bit_extract(src->val(), shift->val().type().width() - 1, 1);
