...
void binop_translator::do_translate()
{
	...
	case XED_ICLASS_SUBSD: {
		...
		auto op0_low = builder().insert_bitcast(value_type(value_type_class::floating_point, size), builder().insert_bit_extract(op0->val(), 0, size)->val());
	...
	case XED_ICLASS_PCMPGTD: {
		...
		auto cst_1 = (inst_class == XED_ICLASS_PCMPGTB) ? builder().insert_constant_u8(0xFF)
			: (inst_class == XED_ICLASS_PCMPGTW)		? builder().insert_constant_u16(0xFFFF)
