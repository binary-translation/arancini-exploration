...
void punpck_translator::do_translate()
{
  ...
  switch (inst) {
	...
	case XED_ICLASS_UNPCKHPD: {
		auto v0 = builder().insert_bitcast(value_type::vector(value_type::f64(), 2), op0->val());
		auto v1 = builder().insert_bitcast(value_type::vector(value_type::f64(), 2), op1->val());
		v0 = builder().insert_vector_extract(v0->val(), 1);
		v1 = builder().insert_vector_extract(v1->val(), 1);
		auto dst = builder().insert_bitcast(value_type::vector(value_type::f64(), 2), op0->val());
