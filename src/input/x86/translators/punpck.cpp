...
void punpck_translator::do_translate()
{
	...
	case XED_ICLASS_PUNPCKLQDQ: {
		...
		write_operand(0, builder().insert_vector_insert(dst->val(), 1, builder().insert_vector_extract(src->val(), 0)->val())->val());
	...
	case XED_ICLASS_PUNPCKLDQ: {
		...
		auto src = builder().insert_bitcast(value_type::vector(value_type::u32(), 4), op1->val());

		auto ex_lo = builder().insert_vector_extract(src->val(), 1);
		auto ex_hi = builder().insert_vector_extract(src->val(), 3);

		write_operand(0, builder().insert_vector_insert(builder().insert_vector_insert(dst->val(), 1, ex_lo->val())->val(), 3, ex_hi->val())->val());
		break;
	}
	default:
