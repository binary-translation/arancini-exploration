...
void punpck_translator::do_translate()
{
	...
	case XED_ICLASS_PUNPCKHQDQ: {
		...
		for (int i = 0; i < input_size / elt_size / 2; i++) {
			dst = builder().insert_vector_insert(dst->val(), 2 * i, builder().insert_vector_extract(v0->val(), i + v0->val().type().nr_elements() / 2)->val());
			dst = builder().insert_vector_insert(
				dst->val(), 2 * i + 1, builder().insert_vector_extract(v1->val(), i + v1->val().type().nr_elements() / 2)->val());
		}
