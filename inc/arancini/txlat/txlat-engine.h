...
private:
    ...
    void generate_dot_graph(arancini::output::o_static::static_output_engine &oe, const std::string& filename);
	...
	static void add_symbol_to_output(const std::vector<std::shared_ptr<elf::program_header>> &phbins, const std::map<off_t, unsigned int> &end_addresses,
                                     const elf::symbol &sym, std::ofstream &s, std::map<uint64_t, std::string> &ifuncs,
	...
	static std::map<std::uint64_t, std::string> generate_guest_sections(const std::shared_ptr<util::basefile> &phobjsrc, elf::elf_reader &elf,
