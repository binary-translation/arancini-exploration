/**
 * All of these implementations assume that the widths of the input registers
 * are at least as big as the intended output.
 */
#pragma once
...
inline void add(InstructionBuilder &builder, TypedRegister &out,
                const TypedRegister &lhs, const TypedRegister &rhs) {
    ...
    case value_type_class::unsigned_integer:
        ...
        case 16:
            if (out.type().element_width() == 8) {
                ...
                builder.xor_(out_reg2, src_reg12,
                             src_reg22);          // carryless addition
...
inline void addi(InstructionBuilder &builder, TypedRegister &out,
                 const TypedRegister &lhs, intptr_t imm) {
...
inline void sub(InstructionBuilder &builder, TypedRegister &out,
                const TypedRegister &lhs, const TypedRegister &rhs) {
    switch (out.type().type_class()) {
...
inline void xor_(InstructionBuilder &builder, TypedRegister &out,
                 const TypedRegister &lhs, const TypedRegister &rhs) {
    if (!out.type().is_vector() && out.type().is_integer()) {
        switch (out.type().element_width()) {
        case 1:
        ...
        case 16:
        case 32:
            // In-types might be wider than out-type (but xor preserves
            // extension, so out accurate to narrower of in)
...
inline void mul(InstructionBuilder &builder, TypedRegister &out,
                const TypedRegister &lhs, const TypedRegister &rhs) {
    if (!out.type().is_vector() && out.type().is_integer()) {
        switch (out.type().element_width()) {
        case 128: {
        ...
        case 32:
        case 16:
            builder.mul(out, lhs, rhs); // Assumes proper signed/unsigned
                                        // extension from 32/16/8 bits
            break;
        default:
            throw std::runtime_error("not implemented");
        }
...
inline void div(InstructionBuilder &builder, TypedRegister &out,
                TypedRegister &lhs, TypedRegister &rhs) {
    if (!out.type().is_vector() && out.type().is_integer()) {
        switch (out.type().type_class()) {

        case value_type_class::signed_integer:
            switch (out.type().element_width()) {
            case 128: // Fixme 128 bits not natively supported on RISCV,
                      // assuming just extended 64 bit value
            ...
            case 16:
                ...
                [[fallthrough]];
                // fallthrough (use 32 bit div because it might be faster on
                // hardware)

                // Almost sure that this will mean the result will be correctly
                // sign extended to 64 bit after the divw Only the overflow case
                // of MIN_INT16 / -1 will not be correct but that should crash
...
inline void mod(InstructionBuilder &builder, TypedRegister &out,
                TypedRegister &lhs, TypedRegister &rhs) {
    if (!out.type().is_vector() && out.type().is_integer()) {
        switch (out.type().type_class()) {
        ...
        case value_type_class::signed_integer:
            ...
            case 16:
                ...
                fixup(builder, rhs, rhs, value_type::s32());
                [[fallthrough]];
                // fallthrough (use 32 bit rem because it might be faster on
                // hardware)
            case 32:
        ...
        case value_type_class::unsigned_integer:
            ...
            case 16:
                ...
                // Almost sure that this will mean the result will be correctly
                // sign extended to 64 bit after the remw Only the overflow case
                // of MIN_INT16 % -1 will not be correct but that should crash
                // anyway so not an issue
            case 32:
...
inline void not_(InstructionBuilder &builder, TypedRegister &out,
