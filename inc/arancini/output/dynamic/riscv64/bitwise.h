...
inline void truncate(InstructionBuilder &builder, TypedRegister &out, const TypedRegister &src)
{
	if (!out.type().is_vector() && out.type().is_integer()) {
		const RegisterOperand src_reg = is_i128_t(src.type()) ? src.reg1() : src;
...
inline void bit_extract(InstructionBuilder &builder, TypedRegister &out, const TypedRegister &src, int from, int length)
{
	...
	const RegisterOperand src1 = src.type().width() != 128 ? src : from < 64 ? src.reg1() : src.reg2();
	if (from >= 64) {
		from -= 64;
	}
	...
	builder.srai(out, temp, 64 - length); // Use arithmetic shift to keep sign extension up
	out.set_actual_width();
	out.set_type(value_type::u64());
}
...
inline void bit_insert(InstructionBuilder &builder, TypedRegister &out, const TypedRegister &src, const TypedRegister &bits, int to, const int length)
{
	...
	if (out.type().is_integer()) {
		const RegisterOperand src1 = src.type().width() != 128 ? src : to < 64 ? src.reg1() : src.reg2();
		const RegisterOperand out1 = out.type().width() != 128 ? out : to < 64 ? out.reg1() : out.reg2();
		if (to >= 64) {
			to -= 64;
		}
