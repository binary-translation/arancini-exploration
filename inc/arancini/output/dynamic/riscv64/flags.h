 ...
 */
void zero_sign_flag(InstructionBuilder &builder, TypedRegister &out,
                    RegisterOperand zf, RegisterOperand sf) {
    if (zf || sf) {
...
/**
 * Generate zero, overflow, carry and sign flags based on the result in the
 * given register and the given input registers assuming the operation was an
 * addition.
 * @param zf Destination register of zero flag, pass none_reg to skip
 ...
 * @param sf Destination register of sign flag, pass none_reg to skip
 */
inline void add_flags(InstructionBuilder &builder, TypedRegister &out,
                      TypedRegister &lhs, TypedRegister &rhs,
                      RegisterOperand zf, RegisterOperand of,
    ...
    if (of || cf || zf || sf) {
        if (!out.type().is_vector() && out.type().is_integer()) {
            ...
            if (of) {
                ...
                builder.xor_(of, of,
                             temp); // OF FIXME Assumes out!=lhs && out!=rhs
            }
            if (cf) {
                extend_to_64(builder, rhs, rhs);

                builder.sltu(cf, out, rhs); // CF (Allows typical x86 case of
                                            // lhs==out) FIXME Assumes out!=rhs
            }
...
/**
 * Generate zero, overflow, carry and sign flags based on the result in the
 * given register and the given input register and immediate assuming the
 ...
 * @param sf Destination register of sign flag, pass none_reg to skip
 */
inline void addi_flags(InstructionBuilder &builder, TypedRegister &out,
                       TypedRegister &lhs, intptr_t imm, RegisterOperand zf,
                       RegisterOperand of, RegisterOperand cf,
                       RegisterOperand sf) {
    if (of || cf || zf || sf) {
        if (!out.type().is_vector() && out.type().is_integer()) {
            extend_to_64(builder, out, out);
...
/**
 * Generate zero, overflow, carry and sign flags based on the result in the
 * given register and the given input registers assuming the operation was a
 * subtraction.
 * @param zf Destination register of zero flag, pass none_reg to skip
 * @param of Destination register of overflow flag, pass none_reg to skip
 * @param cf Destination register of carry flag, pass none_reg to skip
 * @param sf Destination register of sign flag, pass none_reg to skip
 */
inline void sub_flags(InstructionBuilder &builder, TypedRegister &out,
                      TypedRegister &lhs, TypedRegister &rhs,
                      RegisterOperand zf, RegisterOperand of,
                      RegisterOperand cf, RegisterOperand sf) {
    if (of || cf || zf || sf) {
 ...
 * @param sf Destination register of sign flag, pass none_reg to skip
 */
inline void subi_flags(InstructionBuilder &builder, TypedRegister &out,
                       TypedRegister &lhs, intptr_t imm, RegisterOperand zf,
                       RegisterOperand of, RegisterOperand cf,
                       RegisterOperand sf) {
    if (of || cf || zf || sf) {
...
/**
 * Generate overflow and carry flags based on the result in the given register
 * and the given input register and immediate assuming the operation was a
 * multiplication.
 * @param of Destination register of overflow flag, pass none_reg to skip
 * @param cf Destination register of carry flag, pass none_reg to skip
 */
inline void mul_flags(InstructionBuilder &builder, TypedRegister &out,
                      RegisterOperand of, RegisterOperand cf) {
            ...
            case 128: {
                ...
                default:
                    throw std::runtime_error(
                        "Unsupported value type for multiply");
                }
            } break;
            case 64:
            case 32:
            case 16: {
                ...
                case value_type_class::signed_integer:
                    if (out.type().element_width() == 64) {
                        ...
                        builder.slli(flag_reg, out,
                                     64 - (out.type().element_width()) / 2);
                        builder.srai(flag_reg, out,
                                     64 - (out.type().element_width()) / 2);
                    }
