...
class riscv64_translation_context : public translation_context {
  public:
    riscv64_translation_context(machine_code_writer &writer)
        : translation_context(writer), assembler_(&writer, true, RV_GC) {}
  ...
  private:
    ...
    std::unordered_map<const ir::label_node *, std::pair<Label *, bool>>
        labels_;
    std::stack<decltype(builder_.next_register().encoding()),
               std::vector<decltype(builder_.next_register().encoding())>>
        idxs_;
    std::forward_list<decltype(builder_.next_register().encoding())>
        live_across_iteration_;

    std::unordered_map<const ir::port *, TypedRegister> treg_for_port_;
    std::forward_list<TypedRegister> temporaries;
    std::unordered_map<const ir::local_var *,
                       std::reference_wrapper<TypedRegister>>
        locals_;
    ...
    std::pair<TypedRegister &, bool>
    allocate_register(const ir::port *p = nullptr,
                      std::optional<RegisterOperand> reg1 = std::nullopt,
                      std::optional<RegisterOperand> reg2 = std::nullopt);
    ...
    TypedRegister &
    materialise_vector_insert(const ir::vector_insert_node &node);
    ...
    allocate_in_order(to_type_t<reg_idx, idx, const port *>... args);
    template <const size_t order[], typename Tuple, size_t... idxs>
    auto reorder(Tuple tuple, std::index_sequence<idxs...>);
