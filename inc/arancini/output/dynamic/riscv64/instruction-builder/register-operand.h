...
public:
	...
	constexpr RegisterOperand(Register reg)
		: encoding_(reg.encoding())
	{
	}
	[[nodiscard]] bool is_virtual() const { return encoding_ >= VIRTUAL_BASE; }

	/// 32-63 are functional registers (e.g. 32-47 the 16 mapped GPRs, 63 unused)

	[[nodiscard]] bool is_functional() const { return encoding_ < VIRTUAL_BASE && encoding_ >= FUNCTIONAL_BASE; }

	[[nodiscard]] bool is_physical() const { return encoding_ < FUNCTIONAL_BASE; }

	operator Register() const { return Register { encoding_ }; }

	[[nodiscard]] uint32_t encoding() const { return encoding_; }

	constexpr bool operator==(const RegisterOperand &rhs) const { return encoding_ == rhs.encoding_; }
	constexpr bool operator!=(const RegisterOperand &rhs) const { return encoding_ != rhs.encoding_; }
	...
	void allocate(uint32_t encoding)
	{
		...
		if (encoding > 31) {
			throw std::runtime_error("Allocating unavailable register at index: " + std::to_string(encoding));
		}
		encoding_ = encoding;
	}
