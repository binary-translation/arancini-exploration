...
using Rs1Rs2LabelBoolFunc = void (Assembler::*)(Register, Register, Label *,
                                                bool);
...
struct Instruction {
    Instruction(const InstructionType type, const ImmFunc immFunc,
                intptr_t align)
        : rd(none_reg), rs1(none_reg), rs2(none_reg), imm(align), type_(type),
          immFunc_(immFunc) {
        ASSERT(!(!rd && has_rd()));
    }

    Instruction(const InstructionType type, const LabelFunc labelFunc,
                Label *label)
        : rd(none_reg), rs1(none_reg), rs2(none_reg), label(label), type_(type),
          labelFunc_(labelFunc) {
        ASSERT(!(!rd && has_rd()));
    }
    Instruction(const InstructionType type, const RdImmFunc rdImmFunc,
                const RegisterOperand rd, const intptr_t imm)
        : rd(rd), rs1(none_reg), rs2(none_reg), imm(imm), type_(type),
          rdImmFunc_(rdImmFunc) {
        ASSERT(!(!rd && has_rd()));
    }
    Instruction(const InstructionType type, const RdImmFunc rdImmFunc,
                const RegisterOperand rd, const RegisterOperand rs1,
                const intptr_t imm)
        : rd(rd), rs1(rs1), rs2(none_reg), imm(imm), type_(type),
          rdImmFunc_(rdImmFunc) {
        ASSERT(!(!rd && has_rd()));
    }
    Instruction(const InstructionType type, const RdLabelFunc rdLabelFunc,
                const RegisterOperand rd, Label *label)
        : rd(rd), rs1(none_reg), rs2(none_reg), label(label), type_(type),
          rdLabelFunc_(rdLabelFunc) {
        ASSERT(!(!rd && has_rd()));
    }
    Instruction(const InstructionType type, const RdRs1ImmFunc rdRs1ImmFunc,
                const RegisterOperand rd, const RegisterOperand rs1,
                const intptr_t imm)
        : rd(rd), rs1(rs1), rs2(none_reg), imm(imm), type_(type),
    ...
    Instruction(const InstructionType type, const RdRs1ImmFunc rdRs1ImmFunc,
                const RegisterOperand rd, const RegisterOperand rs1,
                const RegisterOperand rs2, const intptr_t imm)
        : rd(rd), rs1(rs1), rs2(rs2), imm(imm), type_(type),
          rdRs1ImmFunc_(rdRs1ImmFunc) {
        ASSERT(!(!rd && has_rd()));
    }
    Instruction(const InstructionType type,
                const Rs1Rs2LabelBoolFunc rs1Rs2LabelBoolFunc,
                const RegisterOperand rs1, const RegisterOperand rs2,
                Label *label)
        : rd(none_reg), rs1(rs1), rs2(rs2), label(label), type_(type),
          rs1Rs2LabelBoolFunc_(rs1Rs2LabelBoolFunc) {
        ASSERT(!(!rd && has_rd()));
    }
    Instruction(const InstructionType type,
                const Rs1Rs2LabelFunc rs1Rs2LabelFunc,
                const RegisterOperand rs1, const RegisterOperand rs2,
                Label *label)
        : rd(none_reg), rs1(rs1), rs2(rs2), label(label), type_(type),
    ...
    Instruction(const InstructionType type, const RdAddrFunc rdAddrFunc,
                const RegisterOperand rd, const RegisterOperand rs1,
                const RegisterOperand rs2, const intptr_t imm)
        : rd(rd), rs1(rs1), rs2(rs2), imm(imm), type_(type),
    ...
    Instruction(const InstructionType type,
                const RdAddrOrderFunc rdAddrOrderFunc, const RegisterOperand rd,
                const RegisterOperand rs1, const std::memory_order order)
        : rd(rd), rs1(rs1), rs2(none_reg), order(order), type_(type),
          rdAddrOrderFunc_(rdAddrOrderFunc) {
        ASSERT(!(!rd && has_rd()));
    }
    Instruction(const InstructionType type,
                const RdRs2AddrOrderFunc rdRs2AddrOrderFunc,
                const RegisterOperand rd, const RegisterOperand rs1,
                const RegisterOperand rs2, const std::memory_order order)
        : rd(rd), rs1(rs1), rs2(rs2), order(order), type_(type),
          rdRs2AddrOrderFunc_(rdRs2AddrOrderFunc) {
        ASSERT(!(!rd && has_rd()));
    }
    Instruction(const InstructionType type, const RdRs1Rs2Func rdRs1Rs2Func,
                const RegisterOperand rd, const RegisterOperand rs1,
                const RegisterOperand rs2)
        : rd(rd), rs1(rs1), rs2(rs2), imm(0), type_(type),
          rdRs1Rs2Func_(rdRs1Rs2Func) {
        ASSERT(!(!rd && has_rd()));
    }
    Instruction(const InstructionType type, const NoneFunc noneFunc)
        : rd(none_reg), rs1(none_reg), rs2(none_reg), imm(0), type_(type),
          noneFunc_(noneFunc) {
        ASSERT(!(!rd && has_rd()));
    }
    Instruction(const InstructionType type, const RegisterOperand rd,
                const RegisterOperand rs1, const RegisterOperand rs2)
        : rd(rd), rs1(rs1), rs2(rs2), imm(0), type_(type), noneFunc_(nullptr) {
        ASSERT(!(!rd && has_rd()));
    }
    ...
    [[nodiscard]] bool is_dead() const {
        return type_ == InstructionType::Dead;
    }

    bool is_mv() {
        return ((type_ == InstructionType::RdRs1ImmKeepRs2 ||
                 type_ == InstructionType::RdRs1Imm) &&
                rdRs1ImmFunc_ == &Assembler::addi_normal && imm == 0) ||
               (type_ == InstructionType::RdRs1Rs2 &&
                rdRs1Rs2Func_ == &Assembler::add &&
                (rs1 == ZERO || rs2 == ZERO));
    }
...
#define ehandle_instr(type, opcode) else handle_instr(type, opcode)
        ...
        case InstructionType::RdImm:
            ...
            out << " " << std::dec << "xV" << rd.encoding() << ", " << std::hex
                << "0x" << imm;
            break;
        case InstructionType::RdLabelNear:
        case InstructionType::RdLabelFar:
        ...
        case InstructionType::RdRs1Imm:
            if (rdRs1ImmFunc_ == &Assembler::addi_normal ||
                rdRs1ImmFunc_ == &Assembler::addi_big) {
                out << "addi";
            ...
            out << " " << std::dec << "xV" << rd.encoding() << ", xV"
                << rs1.encoding() << ", " << std::hex << "0x" << imm;
        ...
        case InstructionType::Rs1Rs2LabelFar:
            if (rs1Rs2LabelBoolFunc_ == (Rs1Rs2LabelBoolFunc)&Assembler::bne) {
                out << "bne";
            } else if (rs1Rs2LabelBoolFunc_ ==
                       (Rs1Rs2LabelBoolFunc)&Assembler::beq) {
                out << "beq";
            } else {
                throw std::runtime_error("Unknown Rs1Rs2Label function");
            }
            out << " " << std::dec << "xV" << rs1.encoding() << ", xV"
                << rs2.encoding() << ", " << std::hex << "label" << label;
            break;
        case InstructionType::Rs1Rs2Label:
        ...
        case InstructionType::RdAddr:
            ...
            out << " " << std::dec << "xV" << rd.encoding() << ", " << imm
                << "(xV" << rs1.encoding() << ")";
            break;
        case InstructionType::Rs2Addr:
            ...
            out << " " << std::dec << "xV" << rs2.encoding() << ", " << imm
                << "(xV" << rs1.encoding() << ")";
            break;
        case InstructionType::RdRs1Rs2:
            // clang-format off
        ...
        case InstructionType::RdImmKeepRs1:
            ...
            out << " " << std::dec << "xV" << rd.encoding() << ", " << std::hex
                << "0x" << imm << std::dec << ", keep xV" << rs1.encoding();
            break;
        case InstructionType::RdRs1ImmKeepRs2:
            if (rdRs1ImmFunc_ == &Assembler::addi_normal) {
                out << "addi";
            ...
            out << " " << std::dec << "xV" << rd.encoding() << ", xV"
                << rs1.encoding() << ", " << std::hex << "0x" << imm << std::dec
                << ", keep xV" << rs2.encoding();
            break;
        }
#undef handle_instr
#undef ehandle_instr
