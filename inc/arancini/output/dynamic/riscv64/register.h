 ...
 * Many instructions only work on specific bit widths, so regardless of the IR
 * type we need to expand the types. Tracking the represented type allows to
 * only do these expansions when necessary.
 *
 */
class TypedRegister {

  public:
    explicit TypedRegister(const Register &reg) : encoding_(reg.encoding()) {};

    explicit TypedRegister(uint32_t encoding) : encoding_(encoding) {};

    TypedRegister(const Register &reg1, const Register &reg2)
        : vt_{value_type::u128()}, encoding1_(reg1.encoding()),
          encoding2_(reg2.encoding()) {};

    operator RegisterOperand() const {
        return RegisterOperand{encoding_};
    ...
    [[nodiscard]] RegisterOperand reg() const {
        return RegisterOperand{encoding_};
    }
    [[nodiscard]] RegisterOperand reg1() const {
        return RegisterOperand{encoding1_};
    }
    [[nodiscard]] RegisterOperand reg2() const {
        return RegisterOperand{encoding2_};
    }
    ...
    bool operator==(const TypedRegister &other) const {
        return encoding_ == other.encoding();
    }
    bool operator!=(const TypedRegister &other) const {
        return encoding_ != other.encoding();
    }
    ...
    [[nodiscard]] RegisterOperand flag(uint32_t flag_type) const {
        return type().element_width() == 128
                   ? RegisterOperand{encoding2_ + flag_type + 1}
                   : RegisterOperand{encoding_ + flag_type + 1};
    }

  private:
    /**
     * Current type that the value in the register is accurate to. Assumes the
     * value is represented as signed regardless of the type. Some of the bits
     * may have resulted from sign extensions. Bits in parts of the register
     * unused by this type may be garbage.
     */
    ...
    /// The actual bit width of the represented value. No bits of this come from
    /// sign extension. 0 means not overridden.
