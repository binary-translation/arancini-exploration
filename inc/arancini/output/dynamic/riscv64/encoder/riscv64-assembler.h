...
public:
	...
	explicit Address(Register base)
		: base_(base)
		, offset_(0)
	{
	}
...
public:
	...
	bool Supports(Extension extension) const { return extensions_.Includes(extension); }
	bool Supports(ExtensionSet extensions) const { return extensions_.IncludesAll(extensions); }
	...
	void beqz(Register rs, Label *label, bool near = kFarJump) { beq(rs, ZERO, label, near); }
	void bnez(Register rs, Label *label, bool near = kFarJump) { bne(rs, ZERO, label, near); }
	...
	// ==== RV64A ====
#if XLEN >= 64
	void lrd(Register rd, Address addr, std::memory_order order = std::memory_order_relaxed);
	void scd(Register rd, Register rs2, Address addr, std::memory_order order = std::memory_order_relaxed);
	void amoswapd(Register rd, Register rs2, Address addr, std::memory_order order = std::memory_order_relaxed);
	void amoaddd(Register rd, Register rs2, Address addr, std::memory_order order = std::memory_order_relaxed);
	void amoxord(Register rd, Register rs2, Address addr, std::memory_order order = std::memory_order_relaxed);
	void amoandd(Register rd, Register rs2, Address addr, std::memory_order order = std::memory_order_relaxed);
	void amoord(Register rd, Register rs2, Address addr, std::memory_order order = std::memory_order_relaxed);
	void amomind(Register rd, Register rs2, Address addr, std::memory_order order = std::memory_order_relaxed);
	void amomaxd(Register rd, Register rs2, Address addr, std::memory_order order = std::memory_order_relaxed);
	void amominud(Register rd, Register rs2, Address addr, std::memory_order order = std::memory_order_relaxed);
	void amomaxud(Register rd, Register rs2, Address addr, std::memory_order order = std::memory_order_relaxed);
#endif // XLEN >= 64

#if XLEN == 32
	void lr(Register rd, Address addr, std::memory_order order = std::memory_order_relaxed) { lrw(rd, addr, order); }
	void sr(Register rd, Register rs2, Address addr, std::memory_order order = std::memory_order_relaxed) { scw(rd, rs2, addr, order); }
#elif XLEN == 64
	void lr(Register rd, Address addr, std::memory_order order = std::memory_order_relaxed) { lrd(rd, addr, order); }
	void sr(Register rd, Register rs2, Address addr, std::memory_order order = std::memory_order_relaxed) { scd(rd, rs2, addr, order); }
#elif XLEN == 128
	void lr(Register rd, Address addr, std::memory_order order = std::memory_order_relaxed) { lrq(rd, addr, order); }
	void sr(Register rd, Register rs2, Address addr, std::memory_order order = std::memory_order_relaxed) { scq(rd, rs2, addr, order); }
#endif
...
private:
	...
	uint32_t Read32(intptr_t position) { return *reinterpret_cast<uint32_t *>(reinterpret_cast<uword>(writer->ptr()) + position); }
	void Write32(intptr_t position, uint32_t instruction) { *reinterpret_cast<uint32_t *>(reinterpret_cast<uword>(writer->ptr()) + position) = instruction; }
	void Emit32(uint32_t instruction)
	{
		instructions32++;
		writer->emit32(instruction);
	}
	uint16_t Read16(intptr_t position) { return *reinterpret_cast<uint16_t *>(reinterpret_cast<uword>(writer->ptr()) + position); }
	void Write16(intptr_t position, uint16_t instruction) { *reinterpret_cast<uint16_t *>(reinterpret_cast<uword>(writer->ptr()) + position) = instruction; }
