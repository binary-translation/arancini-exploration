...
inline int32_t SignExtend(int N, int32_t value) { return (value << (32 - N)) >> (32 - N); }

inline intx_t sign_extend(int32_t x) { return static_cast<intx_t>(x); }
inline intx_t sign_extend(int64_t x) { return static_cast<intx_t>(x); }
inline intx_t sign_extend(uint32_t x) { return static_cast<intx_t>(static_cast<int32_t>(x)); }
inline intx_t sign_extend(uint64_t x) { return static_cast<intx_t>(static_cast<int64_t>(x)); }

#define DEFINE_TYPED_ENUM_SET(name, storage_t)                                                                                                                 \
	class name##Set;                                                                                                                                           \
	class name {                                                                                                                                               \
	public:                                                                                                                                                    \
		constexpr explicit name(storage_t encoding)                                                                                                            \
			: encoding_(encoding)                                                                                                                              \
		{                                                                                                                                                      \
		}                                                                                                                                                      \
		constexpr storage_t encoding() const { return encoding_; }                                                                                             \
		constexpr bool operator==(const name &other) const { return encoding_ == other.encoding_; }                                                            \
		constexpr bool operator!=(const name &other) const { return encoding_ != other.encoding_; }                                                            \
	...
	inline std::ostream &operator<<(std::ostream &stream, const name &element) { return stream << #name << "(" << element.encoding() << ")"; }                 \
	class name##Set {                                                                                                                                          \
	public:                                                                                                                                                    \
		...
		constexpr explicit name##Set(storage_t encoding)                                                                                                       \
			: encoding_(encoding)                                                                                                                              \
		{                                                                                                                                                      \
		}                                                                                                                                                      \
		constexpr static name##Set Empty() { return name##Set(0); }                                                                                            \
		constexpr bool Includes(const name r) const { return (encoding_ & (1 << r.encoding())) != 0; }                                                         \
		constexpr bool IncludesAll(const name##Set other) const { return (encoding_ & other.encoding_) == other.encoding_; }                                   \
		constexpr bool IsEmpty() const { return encoding_ == 0; }                                                                                              \
		constexpr bool operator==(const name##Set &other) const { return encoding_ == other.encoding_; }                                                       \
		constexpr bool operator!=(const name##Set &other) const { return encoding_ != other.encoding_; }                                                       \
		constexpr name##Set operator|(const name &other) const { return name##Set(encoding_ | (1 << other.encoding())); }                                      \
		constexpr name##Set operator|(const name##Set &other) const { return name##Set(encoding_ | other.encoding_); }                                         \
		constexpr name##Set operator&(const name##Set &other) const { return name##Set(encoding_ & other.encoding_); }                                         \
                                                                                                                                                               \
	private:                                                                                                                                                   \
		const storage_t encoding_;                                                                                                                             \
	};                                                                                                                                                         \
	constexpr name##Set name::operator|(const name &other) const { return name##Set((1u << encoding_) | (1u << other.encoding_)); }                            \
	constexpr name##Set name::operator|(const name##Set &other) const { return other | *this; }
...
static constexpr RegisterSet kVolatileRegisters = RA | A0 | A1 | A2 | A3 | A4 | A5 | A6 | A7 | T0 | T1 | T2 | T3 | T4 | T5 | T6;
static constexpr RegisterSet kPreservedRegisters = S1 | S2 | S3 | S4 | S5 | S6 | S7 | S8 | S9 | S10 | S11;
...
COMPILE_ASSERT((kVolatileRegisters | kPreservedRegisters | kReservedRegisters) == kAllRegisters);
...
static constexpr FRegisterSet kPreservedFRegisters = FS0 | FS1 | FS2 | FS3 | FS4 | FS5 | FS6 | FS7 | FS8 | FS9 | FS10 | FS11;
...
#define DEFINE_REG_ENCODING(type, name, shift)                                                                                                                 \
	...
	inline type Decode##name(uint32_t encoding) { return type((encoding >> shift) & 31); }
...
#define DEFINE_FUNCT_ENCODING(type, name, shift, mask)                                                                                                         \
	...
	inline type Decode##name(uint32_t encoding) { return static_cast<type>((encoding >> shift) & mask); }
...
inline bool IsBTypeImm(intptr_t imm) { return Utils::IsInt(12, imm) && Utils::IsAligned(imm, 2); }
...
inline bool IsJTypeImm(intptr_t imm) { return Utils::IsInt(20, imm) && Utils::IsAligned(imm, 2); }
...
inline bool IsITypeImm(intptr_t imm) { return Utils::IsInt(12, imm); }
inline uint32_t EncodeITypeImm(intptr_t imm)
{
	ASSERT(IsITypeImm(imm));
	return imm << 20;
}
inline intptr_t DecodeITypeImm(uint32_t encoded) { return SignExtend(12, encoded >> 20); }

inline bool IsUTypeImm(intptr_t imm) { return Utils::IsInt(32, imm) && Utils::IsAligned(imm, 1 << 12); }
inline uint32_t EncodeUTypeImm(intptr_t imm)
{
	ASSERT(IsUTypeImm(imm));
	return imm;
}
inline intptr_t DecodeUTypeImm(uint32_t encoded) { return SignExtend(32, encoded & ~((1 << 12) - 1)); }

inline bool IsSTypeImm(intptr_t imm) { return Utils::IsInt(12, imm); }
...
inline bool IsCInstruction(uint16_t parcel) { return (parcel & 3) != 3; }

class Instruction {
public:
	explicit Instruction(uint32_t encoding)
		: encoding_(encoding)
	{
	}
...
#define DEFINE_REG_PRIME_ENCODING(type, name, shift)                                                                                                           \
	inline uint32_t Is##name(type r) { return (r.encoding() >= 8) && (r.encoding() < 16); }                                                                    \
	inline uint32_t Encode##name(type r)                                                                                                                       \
	{                                                                                                                                                          \
		ASSERT(Is##name(r));                                                                                                                                   \
		return (r.encoding() & 7) << shift;                                                                                                                    \
	}                                                                                                                                                          \
	inline type Decode##name(uint32_t encoding) { return type(((encoding >> shift) & 7) + 8); }
...
inline bool IsCSPLoad4Imm(intptr_t imm) { return Utils::IsUint(8, imm) && Utils::IsAligned(imm, 4); }
...
inline bool IsCSPLoad8Imm(intptr_t imm) { return Utils::IsUint(9, imm) && Utils::IsAligned(imm, 8); }
...
inline bool IsCSPStore4Imm(intptr_t imm) { return Utils::IsUint(8, imm) && Utils::IsAligned(imm, 4); }
...
inline bool IsCSPStore8Imm(intptr_t imm) { return Utils::IsUint(9, imm) && Utils::IsAligned(imm, 8); }
...
inline bool IsCMem4Imm(intptr_t imm) { return Utils::IsUint(7, imm) && Utils::IsAligned(imm, 4); }
...
inline bool IsCMem8Imm(intptr_t imm) { return Utils::IsUint(8, imm) && Utils::IsAligned(imm, 8); }
...
inline bool IsCJImm(intptr_t imm) { return Utils::IsInt(11, imm) && Utils::IsAligned(imm, 2); }
...
inline bool IsCBImm(intptr_t imm) { return Utils::IsInt(8, imm) && Utils::IsAligned(imm, 2); }
...
inline bool IsCIImm(intptr_t imm) { return Utils::IsInt(6, imm); }
...
inline bool IsCUImm(intptr_t imm) { return Utils::IsInt(17, imm) && Utils::IsAligned(imm, 1 << 12); }
...
inline bool IsCI16Imm(intptr_t imm) { return Utils::IsInt(10, imm) && Utils::IsAligned(imm, 16); }
...
inline bool IsCI4SPNImm(intptr_t imm) { return Utils::IsUint(9, imm) && Utils::IsAligned(imm, 4); }
...
class CInstruction {
public:
	explicit CInstruction(uint16_t encoding)
		: encoding_(encoding)
	{
	}
