...
class register_sequence {
  public:
    ...
    register_sequence(const register_operand &reg) : regs_{reg} {}

    register_sequence(std::initializer_list<register_operand> regs)
        : regs_(regs) {}

    template <typename It>
    register_sequence(It begin, It end) : regs_(begin, end) {}

    operator register_operand() const {
        [[unlikely]]
        if (regs_.size() > 1)
            throw backend_exception(
                "Accessing register set of {} registers as single register",
                regs_.size());
        return regs_[0];
    }

    operator std::vector<register_operand>() const { return regs_; }

    [[nodiscard]]
    register_operand &operator[](std::size_t i) {
        return regs_[i];
    }

    [[nodiscard]]
    const register_operand &operator[](std::size_t i) const {
        return regs_[i];
    }

    [[nodiscard]]
    register_operand &front() {
        return regs_[0];
    }

    [[nodiscard]]
    const register_operand &front() const {
        return regs_[0];
    }

    [[nodiscard]]
    register_operand &back() {
        return regs_[regs_.size()];
    }

    [[nodiscard]]
    const register_operand &back() const {
        return regs_[regs_.size()];
    }
...
class virtual_register_allocator {
  public:
    ...
    [[nodiscard]]
    register_sequence &get(const ir::port &p) {
        return port_to_vreg_[&p];
    }
...
class arm64_translation_context : public translation_context {
  public:
    ...
    virtual void begin_instruction(off_t address,
                                   const std::string &disasm) override;
  ...
  private:
    ...
    std::unordered_map<const ir::local_var *, std::vector<register_operand>>
        locals_;
    ...
    memory_operand
    guestreg_memory_operand(int regoff,
                            memory_operand::address_mode mode =
                                memory_operand::address_mode::direct);
    ...
    register_operand
    add_membase(const register_operand &addr,
                const ir::value_type &t = ir::value_type::u64());
