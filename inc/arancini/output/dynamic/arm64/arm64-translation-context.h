...
private:
	...
	vreg_operand alloc_vreg(ir::value_type type) { return vreg_operand(next_vreg_++, type); }
	...
	memory_operand guestreg_memory_operand(int regoff, bool pre = false, bool post = false);
	...
	template <typename T, std::enable_if_t<std::is_arithmetic<T>::value, int> = 0> vreg_operand mov_immediate(T imm, ir::value_type type);
