...
#define DEFREG(ctype, ltype, name) { X86_OFFSET_OF(name), __COUNTER__ - counter_base1_ - 1 },
#include <arancini/input/x86/reg.def>
#undef DEFREG
	};
...
#undef DEFREG
	...
	static unsigned long offset_to_idx(reg_offsets reg) { return arancini::input::x86::off_to_idx.at((unsigned long)reg); }

	static const char *offset_to_name(reg_offsets reg) { return arancini::input::x86::off_to_name.at((unsigned long)reg); }
